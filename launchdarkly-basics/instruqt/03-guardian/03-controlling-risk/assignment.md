...
notes:
- type: text
  contents: You've seen how quickly you can release new features, delivering them
...
A *segment* is a collection of users, devices, or any other group that share some sort of common attribute. By creating a segment, you can provide targeted access to specific features. In the previous lab, you learned how to turn on and off features in a basic way. Now let's reduce our target audience to just your developers who will be testing this app.
...
1. For **Rule 1**, select the following values:
   ...
   1. **Values**:
      ...
      1. april *\<Enter\>*
      1. andy *\<Enter\>*
2. Click **Save changes**, then **Save changes** again.
...
# Next: Test in real time

Now let's update your feature flag from Lab 1 to use our new segment.

1. Click **Flags** in the left navigation menu.
2. Click **Release Updated Storefront**.
3. Under Targeting Configuration for Test, click **View targeting rules**.
4. Click the **+** button (Add rule) and choose **Target segments**.
5. Select the following values:
   1. **Operator**: is in
   1. **Segments**: Developers
   1. **Rollout**: Store Enabled
6. In the **Default rule** section, click the **Edit** button to the right side of the heading:
   1. **Serve --> Variation**: Store Disabled
7. Click the On/Off toggle at the top left to turn the flag **On**.
