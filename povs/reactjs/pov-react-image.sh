...
mkdir -p /opt/react
cd /opt/react
git clone https://github.com/launchdarkly-labs/ld-sample-app-react.git
...
RestartSec=1
User=root
WorkingDirectory=/opt/react/ld-sample-app-react
ExecStart=/usr/bin/npm start --prefix /opt/react/ld-sample-app-react
...
RestartSec=1
User=root
ExecStart=/usr/bin/code-server --host 0.0.0.0 --port 8080 --auth none /opt/react/
